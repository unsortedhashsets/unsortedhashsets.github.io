...
const Minesweeper: React.FC = () => {
  ...
  useEffect(() => {
    if (hasLost) {
      ...
      setFace(Face.lost);
      logger.log(
        `lost the game with result: ${time} seconds and mines: ${mines} on field`
      );
    }
  }, [hasLost, mines, time]);
  ...
  const checkIfWon = (): void => {
    let safeOpenCellsExist = false;
    for (let i = 0; i < cells.length; i++) {
      for (let j = 0; j < cells[i].length; j++) {
        if (
          cells[i][j].value !== CellValue.mine &&
  ...
  return (
    <div className='ms-app'>
      ...
      <div className='ms-body'>{renderCells()}</div>
    </div>
  );
};
